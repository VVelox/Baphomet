...
- **found** ... a rule matched a line. Written on every match, whether or
  ...
  what tripped what.
- **consign** ... an IP was successfully sent below to Kur.
...
A **consign** event adds `.ip` and `.ban_time`, and `.recidive` is true
when it is a seventh-gate escalation to the recidive kur. A consign
...
escalation is the bare consignment.

## Reading it

```shell
# every consignment, as ip and kur
jq -r 'select(.event_type=="consign") | "\(.kur) \(.ip)"' /var/log/baphomet/eve.json
...
- It is telemetry, never load bearing... a write failure is logged and
  shrugged off, it never keeps an IP from being consigned.
