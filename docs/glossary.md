...
of the dead. Every borrowed name stands for a plain idea, and this is the
one place to look any of them up.
...
- **kur** (lowercase, a config unit) :: a named group of watchers that
  ...
  galla runs per kur. See [configuration](configuration.md).
...
- **observe mode** :: `eve_only`, the verdict withheld. A rule, watcher,
  kur, or whole deployment that matches and writes to EVE but banishes
  nobody, a would-be ban surfacing as an `alert`. Stand a new rule up,
  watch what it would have done, then let it act. See [rules](rules.md).
- **gate** :: an optional test layered over a matched line that can still
  ...
  fails a gate is not an offense. See [rules](rules.md).
- **mark** :: a named, expiring brand a rule leaves on a key (an IP, a
  ...
  xbits and flexbits. See [rules](rules.md).
...
- **state tablet / clay tablet** :: a galla's memory: the hit counters,
  ...
  banishment ledger, written so state survives a restart. On disk under
  the tablet dir by default, and pluggable elsewhere via the
  `[ClayTablet]` config table. See [tablets](tablets.md).
- **recidive / recidivist** :: the repeat-offender escalation. An IP
  banished too many times across all kurs is dragged through the seventh
