...
| filter (`filter.d/*.conf`)                             | a rule (`rules/<type>/<name>.yaml`)... some shipped rules cite the fail2ban filter they were translated from             |
| `failregex`                                            | `message_regexp` (syslog/raw/http_error types) or `match` entries (http/json types)                                      |
| `ignoreregex`                                          | `ignore_regexp` / `ignore`                                                                                               |
| `<HOST>`                                               | `%%%%SRC%%%%` etc... or nothing at all for the http/http_error/json types, where the parser already extracted the client |
| `maxretry`                                             | `max_score`                                                                                                              |
| `findtime`                                             | `find_time`                                                                                                              |
...
| action (`action.d/*.conf`)                             | Ereshkigal's kur backends...                                                                                             |
| a jail's several actions (`banaction` + mail + report) | a fan_out gate on the Ereshkigal side...                                                                                 |
...
| the fail2ban SNMP extend for LibreNMS                  | `baphomet lnms-f2b-extend`                                                                                               |
...
- **Weighted scoring** fail2ban counts every match as
...
- **Observe mode** A rule, watcher, kur, or the whole
...
A jail leaning on `usedns` maps to the layered `usedns` setting... `no` /
`resolve_seen` / `resolve_ban`, watcher over kur over global, behind the
global `enable_dns` consent (see [usedns](usedns.md)). fail2ban's `raw`
mode does not exist... Ereshkigal takes addresses, not names... and the
default is `no`, so a hostname banishes nothing until an operator opts
into resolution knowingly.
...
[rules](rules.md) and [configuration](configuration.md)). A filter riding
F-MLFID session tracking ports as envelope-keyed correlation
(`key: [ syslog.host, syslog.daemon, syslog.pid ]`), and the sshd
ddos/extra/aggressive modes built on it already ship as the opt-in
`syslog/sshd-ddos`, `syslog/sshd-extra`, and `syslog/sshd-aggressive`
rules, no porting needed. The [rules-catalog](rules-catalog.md) lists what
already ships, and the handful deliberately not ported and why.
